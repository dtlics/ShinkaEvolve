...
.claude/skills/shinka-orchestrator/subagents/: archive-analyst.md debug-agent.md
orchestrator/tests/: smoke_test.py test_improvements.py test_parity.py
orchestrator docs: .claude/skills/shinka-orchestrator/SKILL.md orchestrator/NOTES.md
...
const DIMENSIONS = [
  {
    key: 'boot-objective-setup',
    title: 'Boot, objective authoring, and leak-proof-evaluator task setup',
    files: 'orchestrator/scripts/construct_mutation_prompt.py, orchestrator/scripts/mutate.py, orchestrator/harness/run_window.py (main: sentinel guard + task_sys_msg/objective_brief threading), .claude/skills/shinka-setup/SKILL.md, .claude/skills/shinka-convert/SKILL.md',
    docs: '.claude/skills/shinka-orchestrator/SKILL.md "Boot" section, CLAUDE.md boot guidance',
    spec: `INTENDED: At boot the orchestrator authors task_sys_msg = goal + hard constraints + the SHAPE of the score + an abstract runtime caution, and optionally task.objective_brief = a qualitative "what we optimize + constraints + native ops" gloss that renders next to the live metric numbers in every prompt. LEAK-PROOFING IS THE EVALUATOR'S JOB, set at task SETUP, not an inner-loop concern: held-out / gate-defining numbers go under the evaluator's private metrics dict (only public metrics reach the prompt via perf_str), so any candidate that passes and improves the metric is by construction good. Evaluator text feedback is ALWAYS fed to the mutation/fix/meta prompts — there is NO use_text_feedback gate, no _strip_eval_text/_EVAL_TEXT_KEYS, and no boot spoiling self-check (all removed). The harness REFUSES to start if task_sys_msg is missing/empty or still equals the sentinel __UNSET_AUTHOR_AT_BOOT__ (unless task.require_sys_msg:false, or --warmup which flips it off for the throwaway run). The system message must actually be injected into EVERY mutation prompt.
KEY CHECKS: (a) Is the sentinel guard real, correctly placed (before any LLM spend), and does the sentinel string match exactly? (b) Does require_sys_msg default true, and does --warmup actually flip it off? (c) Trace task_sys_msg + objective_brief from run.json → construct_mutation_prompt → sampler → mutate: are they truly threaded into every mutation/fix prompt? (d) Confirm NO spoil apparatus survives anywhere (no use_text_feedback key on the orchestrator surface, no _strip_eval_text/_EVAL_TEXT_KEYS, no boot spoiling self-check) and that evaluator feedback is unconditionally fed. (e) Do the docs + setup skills teach the leak-proof-evaluator design — held-out numbers under private metrics, only public surfaced — so "any passing+improving candidate is good"? (f) Do the setup skills emit the sentinel + teach the author-at-boot step + the leak-proof evaluator contract?`,
  },
  {
    ...
    docs: '.claude/skills/shinka-orchestrator/SKILL.md "Failure handling: truthful recording + repair mode", "How you launch"',
...
KEY CHECKS: (a) Confirm the apply-exhausted slot is NOT stored as a parent copy and does NOT receive a positive/fabricated reward (the historical "F-INNER-1" bug) — read the exact branch in run_window where applied is False. (b) Confirm the bandit/reward update on an exhausted slot charges cost only. (c) Confirm an eval-failed child IS archived with its error metadata in the correct island (so repair mode can later target it). (d) Confirm max_patch_attempts and fix_retry_budget are wired and the error text is actually fed back. (e) Confirm every call's cost lands in the ledger. (f) Robustness: model returns applied:true but an empty/no-op diff; missing <NAME>/<DESCRIPTION>; first window with an empty archive.`,
  },
  {
    ...
    docs: '.claude/skills/shinka-orchestrator/SKILL.md concern map "Exploration / parent", config levers parent_selection_lambda / validity_floor / island_selection_strategy / enforce_island_separation',
...
KEY CHECKS: (a) Does parent selection actually weight by score in the documented direction (higher score → more likely), and does parent_selection_lambda sharpen as described? (b) Does validity_floor behave as documented (inert by default; floors valid parents when set)? (c) Does enforce_island_separation actually toggle same-island vs cross-island inspirations, and are inspirations drawn from the parent's island by default? (d) Edge cases: empty or single-program island; an all-zero-score (flat) archive — does selection degrade gracefully or crash / always pick index 0? (e) Does select:"errored" return an errored parent with NO inspirations (per the repair-mode contract)? (f) Are inspiration count / ordering deterministic and sane?`,
  },
  {
    ...
    docs: '.claude/skills/shinka-orchestrator/SKILL.md diversity/novelty rows, levers code_embed_sim_threshold / reward_on_reject, diagnostics novelty_rejected_cost / novelty_acceptance_rate',
...
KEY CHECKS: (a) Does meta actually emit one DISTINCT direction per live island (count + differentiation), and does run_window persist each into the correct island's brief? (b) Does meta receive the live island list + the current archive as context? (c) Does the "assign already-working directions to existing program entries" behavior EXIST in code, or is it only the user's intent (a design-gap)? (d) Is a failure_note produced and surfaced? (e) Does auto_meta:false suppress BOTH the global directions AND the per-island briefs (islands keep their last brief)? (f) Is a meta exception truly swallowed (window continues)? (g) Is meta cost logged once (self-log) and not also appended as an intervention?`,
  },
  {
    ...
    docs: '.claude/skills/shinka-orchestrator/SKILL.md "Islands differentiate BY DEFAULT", levers brief_compose_mode / meta_directions / island_selection_strategy',
...
KEY CHECKS (end-to-end producer→consumer): (a) Does the code ACTUALLY switch from random-code inspirations to direction-sampled inspirations once an island has a brief — or does it still sample random code and merely append the brief text (a half-wired design-gap)? (b) Is exactly ONE direction sampled per generation for the parent's island? (c) When code entries correspond to the sampled direction, are they attached — and HOW is "corresponds" determined (is there even a mapping from direction→programs)? (d) Does brief_compose_mode=replace actually replace the global direction for that island? (e) Trace the full path: meta writes brief → stored where → read by which sampler/prompt builder → reaches the mutation prompt. Flag any break in this chain.`,
  },
  {
    ...
    docs: '.claude/skills/shinka-orchestrator/SKILL.md "Island structure" concern + spawn_island paragraph, levers max_islands / island_evict_strategy / enable_dynamic_islands / migration_interval / island_policy_driven',
...
KEY CHECKS: (a) Dead-code: is island_policy.py actually invoked on any DEFAULT route, or only when island_policy_driven:true (default false) — and is island_policy.main() reachable at all? (b) Does per-island eviction remove the WORST members (by the configured strategy), not arbitrary ones? (c) Does spawn_island enforce max_islands + worst-island retirement + the island-0/global-best protections exactly? (d) When an island is over capacity, are tombstones reclaimed before live programs? (e) Is "retire island" actually EXECUTED or only recommended (NOTES.md hints shinka has no native retire path)? (f) Migration semantics: does it copy/move the right programs at the right cadence, and what does it stamp as the generation?`,
  },
  {
    ...
    docs: '.claude/skills/shinka-orchestrator/SKILL.md "Deep research (the DR check on control-return)"',
    spec: `INTENDED: DR is an ORCHESTRATOR decision at a control-return (NOT a config cadence). The agent writes the query (general SOTA for the task/sub-task, with a citation; never "reproduce paper X verbatim" — that trips Azure's content filter). DR returns directions. Per technique the agent triages: NOVEL (nothing in the archive/prior directions resembles it) → GROUND it with a web-search run → archive_record parent_id=null → give it its OWN island via spawn_island; HISTORY-SIMILAR → COMBINE it into the closest existing program via a grounding run → archive_record parent_id=closest (a lineage child in the SAME island), NO spawn_island, the existing program left intact; otherwise IGNORE. A refused/failed DR call returns refused:true + a reason, logged with the query intact and NO crash. Cost self-logs via results_dir, is budget-gated, and is NOT double-counted.
KEY CHECKS: (a) Does deep_research.main return directions in a usable, documented shape? (b) Does the refusal/exception path return refused:true + reason, log the query, and not crash (the historical "DR terminal failure drops cost / crashes" risk)? (c) Is the web_search tool actually passed to the DR client, and is the long timeout set? (d) Is the 3-scenario triage supported by tooling (spawn_island for NOVEL; a grounding-run config for COMBINE) — or is any of it doc-only with no mechanism? (e) Confirm there is NO automated similarity helper (the doc says the agent judges novelty itself) — and that the docs make that judgment teachable. (f) Is DR cost captured + the per-call cap applied?`,
  },
  {
    ...
    docs: '.claude/skills/shinka-orchestrator/SKILL.md "The taper", "The run loop, end to end", cadence config block',
...
KEY CHECKS: (a) Does cadence_policy compute the next cluster size from the work-score low-streak as documented (monotone escalation; the 1→5→10→20 shape or whatever it actually is — flag any mismatch)? (b) Does a stagnation flag short-circuit the cluster mid-way? (c) Does a MISSING work score default to wake-every-window (not to a large cluster)? (d) Is the taper truly uncapped unless max_windows_per_call is set? (e) Resume / counting: are window_index and prior_low_streak carried correctly across clusters and across --resume (no off-by-one, no double-count, no reset of the streak that would re-trigger stagnation immediately)?`,
  },
  {
    ...
    docs: '.claude/skills/shinka-orchestrator/SKILL.md "The framework-audit rewrite cycle", "tiered mutability", "What never to do"',
...
KEY CHECKS: (a) Does restore_state rewind code+DB+bandit while PROVABLY preserving the cost ledger? (b) Does decide() truly fail closed on crash / empty / NaN (return regressed=true)? Verify the collapse-trigger threshold is actually reachable (the historical "0.85 > 1−epsilon ⇒ dead trigger" concern — check bandit_collapse_count_frac vs the counts signal, not weights). (c) Is the rejected-hash guard enforced in BOTH deploy and deploy_bundle? (d) Does validate smoke ALL of a target's modes (e.g. select_llm's select+weights+update) so a rewrite that breaks the bandit-state snapshot is caught pre-deploy? (e) Does snapshot refuse to run while a window subprocess is live? (f) Foundation protection: is it enforced anywhere in code (can deploy() be pointed at a foundation file?), or purely doc convention? (g) Bundle atomicity on partial failure.`,
  },
  {
    ...
    docs: '.claude/skills/shinka-orchestrator/SKILL.md "Safety railguards"',
...
KEY CHECKS: (a) Is _write_json_atomic genuinely atomic (write tmp, fsync?, os.replace) — or can a crash mid-write truncate run.json? (b) Does read_run actually RECOMPUTE from the streams on a corrupt/missing run.json, and does the recompute sum the SAME set of costs the live ledger does (no missed stream)? (c) Is the budget check placed so overshoot is bounded to one slot (before launching vs after recording)? (d) Is the per-call max-output cap actually applied to EACH call kind (mutate/meta/DR/fix)? (e) Is there ANY LLM call path whose cost is not added to the ledger (an unmonitored call)? (f) Double-count risk: any place that both self-logs AND appends the same cost? (g) Is embedding cost captured on BOTH accept and reject paths (or is the known reject-only asymmetry the only gap)?`,
  },
  {
    ...
    docs: '.claude/skills/shinka-orchestrator/SKILL.md "Termination + end of run"',
    spec: `INTENDED: Stop when (a) budget exhausted, (b) the user says stop, or (c) FIVE consecutive control-returns were EACH stagnant AND had an intervention → stop. This is HARNESS-computed and auto-finalized: journal.termination_streak counts trailing control_return rows that are both stagnation_flag=true and intervened=true; when it reaches cadence.termination_streak (default 5) the next --until-decision call returns stagnation_intervention_exhausted and finalizes. "intervention" is INCLUSIVE (a framework rewrite, a DR, OR a deliberate config-lever flip); there is NO "≥1 DR of 5" requirement; the automatic per-window meta round does NOT count; a stagnation-break (flag false) or a no-intervention return RESETS the streak. A pre-assumed/reference score does NOT end the run early. End of run: write an ending document (outcome + "Future fixes for the user before the next run"), seed it from build_run_summary, flip status via finalize_run (the harness auto-finalizes on BOTH budget_exhausted AND stagnation_intervention_exhausted; the agent only finalizes manually for user-stop), then archive_run into orchestrator/run_archive/<run_id>__<finished_at>/.
KEY CHECKS: (a) Is termination HARNESS-computed (journal.termination_streak over the agent's canonical control_return rows) with the redesigned rule — 5 consecutive stagnant+intervened, intervention = rewrite/DR/lever-flip (NO ≥1-DR requirement), meta does not count — and is "intervened" defined IDENTICALLY across SKILL.md/CLAUDE.md/the journal docstring (what resets the streak)? (b) Do build_run_summary / finalize_run / archive_run exist and return/move the right things? (c) Does auto-finalize fire on BOTH budget AND stagnation-intervention exhaustion (and is finalize_run idempotent about finished_at)? (d) Any path where the run could stop EARLY by mistake (e.g. stagnation_flag or low_streak misread as termination, or --until-decision exiting being treated as "done")? (e) Is "do not stop until a criterion is met" enforced or doc-only?`,
  },
  {
    ...
    docs: '.claude/skills/shinka-orchestrator/SKILL.md "Is a model never being picked? (the framework-audit check)", reward floors, levers cost_aware_coef / epsilon / force_explore / llm_subset',
    spec: `INTENDED: The bandit picks a model per candidate and learns each (model, effort) arm separately. reward_validity_floor floors a CORRECT candidate's reward so "correct-but-worse" beats "failed". cost_aware_coef blends reward vs cheapness; epsilon is an exploration floor; force_explore / llm_subset re-open a starved arm. model_collapse is SURFACED (counts-share) and NEVER auto-corrected in steady state — the agent judges "locked out (a reward/selection flaw)" vs "truly bad". (There is NO latency-aware auto-routing — a 2026-05-27 attempt was reverted in 0edea84.)
KEY CHECKS: (a) Does the bandit ENTRENCH the cheapest arm (the historical H3) — i.e. do cost_aware_coef + the reward floor actually prevent a good-but-pricier arm from being starved, or can a couple of early bad draws permanently lock an arm out? (b) Confirm there is NO latency/auto-route logic in select_llm or prioritization (reverted in 0edea84) — flag any doc that still claims it. (c) Does a novelty-rejected or apply-exhausted slot update the arm consistently with the truthful-recording contract (ties to the mutation + novelty dimensions)? (d) Is the (model,effort) arm split correct and stable across resume (get_state/set_state)? (e) Is a single arm's weight really capped at 1−epsilon (making the weights-collapse signal unreachable and counts-share the only faithful one)? (f) reward direction/sign sanity (higher score ⇒ higher reward).`,
  },
  {
    ...
    docs: '.claude/skills/shinka-orchestrator/SKILL.md diagnostics field list + concern map',
...
KEY CHECKS: (a) Does diagnostics.py actually PRODUCE every field the docs tell the agent to read (any phantom field the agent would look for and not find)? (b) Is the stagnation formula EXACTLY the documented one (scale-free, the max() with the floor, the right comparison)? (c) Is errored_fraction tombstone-excluded? Is evaluation_failure_rate post-repair over EVALUATED slots only (apply-exhausted excluded)? (d) Is model_collapse computed on counts, with n_arms_active / collapsed sane? (e) Division-by-zero / empty-window / fresh-archive safety — does the sensor ever crash or emit NaN where the agent (or rollback) reads a number? (f) null-vs-0 discipline (e.g. novelty_acceptance_rate null when no events) so the agent doesn't misread "no data" as "0%".`,
  },
  {
    ...
    docs: '.claude/skills/shinka-orchestrator/SKILL.md "Warmup", the flaw-signals list',
...
const CROSSCUTS = [
 ...
 3) SKILL↔CODE CROSS-CHECK — the NOTES.md "Code ↔ doc consistency contract" (P9-T0) table was REMOVED (NOTES.md is now a per-run scratchpad), so the surviving enforcement is the doc-lint test (orchestrator/tests test_skill_doc_teaches_run_loop_and_roles asserted-phrase set) plus a direct SKILL.md↔code cross-check: for each behavioral claim in SKILL.md's run-loop / subroutine / lever sections, open the cited file:function and confirm the code REALLY does it. Flag any claim whose code reference is wrong/missing or overstates the code.
 ...
 5) MUTUAL DOC CONSISTENCY — CLAUDE.md vs .claude/skills/shinka-orchestrator/SKILL.md vs orchestrator/NOTES.md vs the cnot task README: do defaults, model names/prices, file lists, and behavioral claims agree with each other and with code? (e.g. code_embed_sim_threshold default, repair defaults, num_islands default, the Azure deployment/price table.)
Inventory for reference:\n${INVENTORY}`,
  },
  {
    key: 'teachability',
    title: 'Orchestrator / outer-loop teachability critique (docs-only lens)',
    prompt: `Read ONLY as a fresh, capable agent who has just been handed this repo and told "you are the orchestrator + outer-loop." Read CLAUDE.md, .claude/skills/shinka-orchestrator/SKILL.md, orchestrator/NOTES.md, the four skills/*/SKILL.md, and .claude/skills/shinka-orchestrator/subagents/*.md. For EACH load-bearing decision the agent must make, grade whether the docs make it correctly and unambiguously:
  • author task_sys_msg + an optional objective_brief, and ensure the EVALUATOR is leak-proof at setup (held-out numbers under private metrics, only public surfaced) so any passing+improving candidate is good;
  ...
  • judge termination (the harness-computed 5-consecutive-stagnant+intervened streak; is "intervened" defined identically everywhere — could two careful agents compute it differently?);
