...
| 2026-05-12 | Cleanup C1 — delete dead inner `@backoff` retry layer (was `max_tries=1`, a no-op) across all 5 provider modules; drop `BACKOFF_MAX_*` constants and obsolete `test_llm_retry_constants.py`. 247 lines removed; 469 tests pass. | done | `04c71f4` |
| 2026-05-12 | Cleanup C3 — drop dead `AsyncLLMClient` import from `async_runner.py` (every construction site now uses `AgentLLMClient`). | done | `4c581e0` |
| 2026-05-12 | Cleanup C2 — mark `shinka/llm/providers/openai.py` as legacy-only (the agentic path goes through the agents SDK Runner; `_extract_message_text`/`_extract_thought_text` are only used by `AgentLLMClient._query_via_legacy`). | done | `b55f458` |
| 2026-05-12 | Cleanup C6 — drop stale "fresh-client retry" doc references in `client.py:504` and `async_runner.py:387` left over from Phase F.1's RobustRunner removal. | done | `53d97fb` |
| 2026-05-12 | **Phase E (full) — evaluator wired into the agent loop**. `_make_agent_evaluator` builds an `EvaluatorCallable` closure bound to `scheduler` + `exec_fname` + `results_dir` + `ctx`; `_run_agent_proposal` binds it on `tool_ctx.evaluator` so `evaluate_tool` actually does work. `max_turns` bumped to `max_patch_attempts * 3` for the apply→evaluate→reflect cycle. Default `agentic_tools` widened to `["apply_patch", "evaluate"]`. **Cache-and-skip downstream**: the closure stashes the full `scheduler.run` result on `ctx.last_eval_result`; the orchestrator surfaces it via `meta_patch_data["_cached_eval_results"]`, and `_inject_cached_evaluation_with_slot` creates an `AsyncRunningJob` with `cached_results` set; the monitor + `_process_completed_jobs_safely` short-circuit the scheduler poll/retrieve path. The legacy proposer path (`use_agentic_proposer=False`) is unchanged. 4 new tests; 473 pass. | done | `74c440b` |
| 2026-05-12 | Cleanup C4 — structured Pydantic output for the agentic proposer. New `PatchProposalOutput` model with `name` / `description` fields; passed as `output_type=` on the `Agent`. `_runresult_to_queryresult` surfaces the parsed instance via `QueryResult.final_output_obj`; `_run_agent_proposal` prefers the typed object and falls back to `<NAME>`/`<DESCRIPTION>` regex only when the structured field is missing (non-OpenAI fallback). 3 new tests; 476 pass. | done | `f68bccd` |
| 2026-05-12 | Cleanup C5 — `ShinkaAgentHooks` (SDK-native lifecycle hooks) replaces the manual `record_tool_call` plumbing. `on_tool_start` clears `ctx.last_tool_extras`; `on_tool_end` infers success from the return-string prefix (`"OK"` / `"Error:"` / `"FAILED:"`), records latency from a FIFO start-time match, and merges `ctx.last_tool_extras` into the trace entry. Each tool dropped its `time.monotonic()`/`record_tool_call` boilerplate; just sets `last_tool_extras` for structured per-call data. Tool unit tests now check the return string + `last_tool_extras` (the hook owns trace population, not exercised when calling `_*_impl` directly). 9 new hook tests; 485 pass. | done | `2c4efc8` |
