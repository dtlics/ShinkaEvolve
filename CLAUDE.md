...
- `o3-deep-research` deployment (Foundry project `dtlics2000-4351`, **westus**), underlying model version `2025-06-26`. Used by `orchestrator/scripts/deep_research.py` (Stage-C DR prompt) via the dedicated `dr_client`. Override the deployment name in that script if you rename it. **The web-search tool spec `{"type":"web_search_preview"}` is CORRECT for the Responses-API path** (it takes NO connection id — that's the Agents API); per Microsoft docs + a verified live call, do NOT swap it to `{"type":"web_search"}` (reported to regress o3-deep-research). A terminal `failed` that still BURNED tokens is therefore NOT a bad tool spec or a dead deployment — it's the web-search backing (**Grounding with Bing Search**) not enabled / provisioned / co-located in the resource's region, a quota/rate cap, or a content-filter trip (→ `incomplete`, `incomplete_details.reason="content_filter"`). Run `python scripts/test_dr.py` to print the real `error.code`/`message`/`incomplete_details.reason` in isolation (verified working 2026-06-10: a real cited brief in ~5 min, ~$0.22 — note the script uses `max_output_tokens=30000` so a trivial probe still returns text rather than capping out empty). A submitted-then-failed DR call is BILLED by Azure even though `usage` is empty — the framework floors its logged cost at `search_surcharge_usd` (≥$0.30) so the ledger reflects the spend.
